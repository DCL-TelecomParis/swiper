...
def solve(inst: Union[WeightRestriction, WeightQualification], no_jit: bool, verify: bool) -> List[int]:
    """
    ...
    if isinstance(inst, WeightQualification):
        return wq_solve(inst, no_jit, verify)
    elif isinstance(inst, WeightRestriction):
        return wr_solve(inst, no_jit, verify)
    else:
        raise ValueError(f"Unknown instance type {type(inst)}")


def wq_solve(inst: WeightQualification, no_jit: bool, verify: bool) -> List[int]:
    """
    Solve the Weight Qualification problem.
    Returns the status of the solution, the solution itself and the gas expended.
    """

    return wr_solve(inst.to_wr(), no_jit, verify)
...
def wr_solve(inst: WeightRestriction, no_jit: bool, verify: bool) -> List[int]:
    ...
    # Then, use actual knapsack to find a local minimum
